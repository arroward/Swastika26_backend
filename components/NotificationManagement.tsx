  ...
  return (
    <div className="w-full text-white">
      <header className="mb-6 sm:mb-8">
        <h2 className="text-2xl sm:text-3xl font-syne font-bold text-white mb-1 sm:mb-2">
          Broadcast Center
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 items-start">
        ...
        <motion.div
          ...
          className="w-full"
        >
          <div className="bg-white/5 p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-white/10 backdrop-blur-sm shadow-2xl">
            ...
            {/* Templates */}
            <div className="mb-8">
              <label className="block text-xs font-mono text-white/30 uppercase tracking-widest mb-3">
                Quick Templates
              </label>
            ...
            <form onSubmit={handleSend} className="space-y-6">
              ...
              {status && (
                ...
                >
                  <p
                    className={`text-center text-sm font-mono ${status.includes("Success") ? "text-green-400" : "text-red-400"}`}
                  >
                    {status}
                  </p>
                    ...
                    <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-lg max-h-40 overflow-y-auto custom-scrollbar">
                      ...
                      <div className="space-y-2">
                        {deliveryErrors.map((err, i) => (
                          <div
                            key={i}
                            className="text-[10px] font-mono text-red-300/70 border-b border-red-500/10 pb-2 last:border-0"
                          >
                            <span className="text-red-400">Token:</span>{" "}
                            {err.token}
                            <br />
                            <span className="text-red-400">Error:</span>{" "}
                            {err.error?.message || "Unknown FCM Error"}
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        >
          ...
          {/* History List */}
          <div className="bg-white/5 p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-white/10 backdrop-blur-sm min-h-[500px]">
        ...
        >
          ...
          >
            ...
            <div className="p-8 border-b border-white/5 flex justify-between items-start sticky top-0 bg-[#111] z-10">
              ...
              <button
                onClick={() => setSelectedNotification(null)}
                className="p-2 hover:bg-white/10 rounded-full transition-colors font-bold text-white"
              >
                ✕
              </button>
            </div>
            ...
            <div className="p-8 space-y-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-green-500/10 p-6 rounded-2xl border border-green-500/20 text-center">
                  <span className="block text-4xl font-bold text-green-400 mb-1">
                    {selectedNotification.successCount ?? "-"}
                  </span>
                  <span className="text-xs font-mono text-green-400/70 uppercase">
                    Delivered
                  </span>
                </div>
                <div className="bg-red-500/10 p-6 rounded-2xl border border-red-500/20 text-center">
                  <span className="block text-4xl font-bold text-red-400 mb-1">
                    {selectedNotification.failureCount ?? "-"}
                  </span>
                  <span className="text-xs font-mono text-red-400/70 uppercase">
                    Failed
                  </span>
                </div>
              </div>
              ...
              {/* Failure Logs */}
              {selectedNotification.failureDetails &&
                selectedNotification.failureDetails.length > 0 && (
                  <div className="space-y-4">

  ...
  const handleDownload = async (format: "csv" | "pdf") => {
    ...
    if (format === "csv") {
      try {
        ...
        link.setAttribute(
          "download",
          `registrations_${cleanTitle}_${filenameDate}.csv`,
        );
        ...
        <div className="bg-white/5 p-6 rounded-3xl border border-white/10 backdrop-blur-sm">
          ...
          <div className="flex items-end gap-2">
            {isLoading ? (
              ...
              <span className="text-4xl font-syne font-bold text-white animate-in fade-in slide-in-from-bottom-2 duration-500 delay-100">
                {events.length}
              </span>
            )}
            <span className="text-xs font-mono text-purple-400 mb-2">
              events
            </span>
          </div>
        </div>

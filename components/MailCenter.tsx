...
import { Mail, Send, Eye, ShieldCheck, AlertCircle, CheckCircle2, Loader2, Info, Users, TestTube2, AtSign } from 'lucide-react';
...
export default function MailCenter() {
    ...
    const [recipientType, setRecipientType] = useState('test'); // 'all', 'test', 'specific'
    ...
    const templates = [
        ...
        },
        {
            ...
            message: "To ensure a smooth experience for everyone, please take a moment to review the latest entry guidelines and safety protocols. Remember to carry a valid ID and your digital pass.",
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        if (!subject || !title || !message) {
            setStatus({ type: 'error', message: 'Subject, Title and Message are required' });
            return;
        }

        if (recipientType === 'specific' && !specificEmail) {
            setStatus({ type: 'error', message: 'Please provide a specific email address' });
            return;
        }
        ...
        try {
            ...
            if (res.ok) {
                setStatus({ type: 'success', message: data.message || 'Broadcast sent successfully!' });
    ...
    return (
        <div className="space-y-8 max-w-6xl mx-auto">
            <header>
                ...
                <p className="text-zinc-400">Broadcast announcements and confirmations via email.</p>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                ...
                >
                    <div className="flex items-center gap-3 border-b border-white/5 pb-4 mb-2">
                        ...
                        <h2 className="text-xl font-bold text-white font-syne uppercase tracking-wider">Compose Mail</h2>
                    </div>

                    {/* Quick Templates */}
                    <div className="bg-white/5 border border-white/5 p-4 rounded-xl">
                        <label className="block text-xs font-mono text-zinc-500 uppercase tracking-widest mb-3">Quick Templates</label>
                    ...
                    <form onSubmit={handleSend} className="space-y-4">
                        <div className="space-y-4">
                            <div>
                                <label className="block text-xs font-mono text-zinc-500 uppercase tracking-widest mb-2">Recipients</label>
                            ...
                            <div>
                                <label className="block text-xs font-mono text-zinc-500 uppercase tracking-widest mb-2">Email Subject</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs font-mono text-zinc-500 uppercase tracking-widest mb-2">Internal Title</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs font-mono text-zinc-500 uppercase tracking-widest mb-2">Message Content</label>
                                <textarea
                                    rows={8}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-mono text-zinc-500 uppercase tracking-widest mb-2">CTA Button Text (Optional)</label>
                                    <input
                                        type="text"
                                ...
                                <div>
                                    <label className="block text-xs font-mono text-zinc-500 uppercase tracking-widest mb-2">CTA URL (Optional)</label>
                                    <input
                                        type="url"
                                        placeholder="https://..."
                                        className="w-full bg-black border border-white/10 rounded-xl px-4 py-3 text-white focus:border-red-500 outline-none transition-all"
                        ...
                        {status && (
                            <div className={`p-4 rounded-xl flex items-start gap-3 ${status.type === 'success' ? 'bg-green-500/10 border border-green-500/20 text-green-400' :
                    ...
                    <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl overflow-hidden shadow-2xl min-h-[600px] flex flex-col">
                        <div className="p-4 bg-zinc-900/50 border-b border-white/10 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <span className="text-[10px] text-zinc-500 font-mono">RENDERING</span>
                            </div>
                        </div>
                        ...
                        <div className="flex-1 bg-[#020202] overflow-y-auto p-4 custom-scrollbar">
                            <div className="max-w-md mx-auto bg-[#0a0a0a] border border-white/10 rounded-xl overflow-hidden shadow-2xl">
                                <div className="bg-gradient-to-b from-zinc-900 to-[#0a0a0a] p-8 text-center border-b border-white/5">
                                    ...
                                    <p className="text-[10px] text-zinc-500 uppercase tracking-[0.3em] mt-2">Official Communication</p>
                                </div>

                                <div className="p-8 space-y-6">
                                    <div className="text-center space-y-2">
                                        <h2 className="text-xl font-bold text-white font-syne">{title || 'Email Title Ready'}</h2>
                                        <div className="h-0.5 w-12 bg-red-500 mx-auto"></div>
                                    </div>

                                    <div className="text-zinc-400 text-sm leading-relaxed space-y-3">
                                        {message ? message.split('\n').map((p, i) => <p key={i}>{p}</p>) : (
                                            <p className="italic opacity-30">Your announcement message will appear here...</p>
                                        )}
                                    </div>
                                ...
                                <div className="p-8 bg-black/50 border-t border-white/5 text-center space-y-4">
                                    ...
                                    <p className="text-[9px] text-zinc-700">© 2026 {SITE_CONFIG.name.split(" ")[0]}. All rights reserved.</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
function RecipientTypeBtn({ active, onClick, icon, label }: { active: boolean, onClick: () => void, icon: React.ReactNode, label: string }) {
    return (
        <button
            type="button"
            onClick={onClick}

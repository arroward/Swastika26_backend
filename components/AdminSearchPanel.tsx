...
import { Search, Loader2, Ticket, ShoppingCart, Calendar, User, Mail, Phone, QrCode, AlertCircle } from 'lucide-react';
...
export default function AdminSearchPanel() {
    ...
    const [searchType, setSearchType] = useState<'all' | 'ticket' | 'purchase' | 'email' | 'phone'>('all');
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await fetch(`/api/admin/search?q=${encodeURIComponent(searchQuery)}&type=${searchType}`);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                <h2 className="text-xl font-bold text-white mb-2">Search Tickets & Purchases</h2>
                <p className="text-gray-400 text-sm">Find tickets and purchases by ID, email, phone, or name</p>
            </div>
                ...
                <div className="flex gap-2 flex-wrap">
                    {[
                        ...
                        <button
                            ...
                            className={`px-3 py-1.5 text-xs font-medium rounded transition-colors ${searchType === type.value
                        ...
                        <div>
                            ...
                            <div className="space-y-2">
                                {results.purchases.map(purchase => (
                                    <PurchaseResultCard key={purchase.purchaseId} purchase={purchase} />
                                ))}
                            </div>
                        </div>
                    )}
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4">
                <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-mono text-sm text-white font-bold">{ticket.ticketId}</span>
                        <span className={`text-xs px-2 py-0.5 rounded ${typeColors[ticket.type]}`}>
                            {ticket.type.replace('_', ' ')}
                        </span>
                        <span className={`text-xs px-2 py-0.5 rounded border ${statusColors[ticket.status]}`}>
                            {ticket.status}
                        </span>
                    </div>
                ...
                <div className="flex flex-col items-end gap-1 text-xs text-gray-500">
                    <div>Created: {new Date(ticket.createdAt?.seconds * 1000).toLocaleDateString()}</div>
                    {ticket.allowedDays && (
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4">
                <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-mono text-sm text-white font-bold">{purchase.purchaseId}</span>
                        <span className={`text-xs px-2 py-0.5 rounded border ${statusColors[purchase.paymentStatus]}`}>
                    ...
                    {purchase.paymentId && (
                        <div className="text-xs text-gray-500">
                            Payment ID: <span className="font-mono">{purchase.paymentId}</span>
                        </div>
                    )}
                    ...
                    {purchase.tickets && purchase.tickets.length > 0 && (
                        <div className="mt-2 text-xs text-gray-500">
                            ...
                            <span className="ml-2 font-mono text-gray-600">
                                ...
                                {purchase.tickets.length > 3 && ` +${purchase.tickets.length - 3} more`}]
                            </span>
                        </div>
                    )}
                </div>
                ...
                <div className="flex flex-col items-end gap-1 text-xs text-gray-500">
                    ...
                    <div className="text-gray-600">
                        {new Date(purchase.purchaseDate?.seconds * 1000).toLocaleTimeString()}
                    </div>
                </div>
            </div>
        </div>

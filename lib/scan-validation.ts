 ...
 */
export function canTransferTicket(ticket: Ticket): {
  allowed: boolean;
  reason?: string;
} {
  ...
  if (ticket.scans.length > 0) {
    return {
      allowed: false,
      reason: "Cannot transfer a ticket that has been scanned",
    };
  }
  return { allowed: true };
}
 ...
 */
export function canCancelTicket(ticket: Ticket): {
  allowed: boolean;
  reason?: string;
} {
  if (ticket.status === "USED") {
  ...
  if (ticket.scans.length > 0) {
    return {
      allowed: false,
      reason: "Cannot cancel a ticket that has been scanned",
    };
  }
  return { allowed: true };
}
...
export function getTicketStatusText(ticket: Ticket): string {
  ...
  if (ticket.scans.length < maxScans)
    return `Partially Used (${ticket.scans.length}/${maxScans})`;
...
export function getScanSummary(ticket: Ticket): {
  ...
  const remainingDays = config.allowedDays.filter(
    (day) => !scannedDays.includes(day),
  );

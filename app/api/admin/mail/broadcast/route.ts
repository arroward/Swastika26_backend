...
export async function POST(request: Request) {
    try {
        const { subject, title, message, ctaText, ctaUrl, recipientType, specificEmail } = await request.json();
        ...
        if (recipientType === 'all') {
            const snapshot = await adminFirestore.collection('purchases').where('paymentStatus', '==', 'COMPLETED').get();
